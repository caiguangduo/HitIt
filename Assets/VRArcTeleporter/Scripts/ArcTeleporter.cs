...
{
    [HideInInspector]
    public static bool IsArcteleped = false;
    ...
    private void CalculateArc()
    {
        ...
        if (_teleportHighlightInstance != null)
        {
            ...
            else
            {
                ...
                Vector3 footDirection = hit.point - transform.position;
                float angleY = Mathf.Atan2(footDirection.x, footDirection.z) * Mathf.Rad2Deg; //朝向
                _teleportHighlightInstance.transform.localEulerAngles = new Vector3(-90, angleY, 0);
            }
        }
    ...
    IEnumerator WaitToCloseTeleport()
    {
